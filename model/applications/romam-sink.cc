...
NS_LOG_COMPONENT_DEFINE("RomamSink");

NS_OBJECT_ENSURE_REGISTERED(RomamSink);

TypeId
RomamSink::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::RomamSink")
            ...
            .AddAttribute("Local",
                          ...
                          AddressValue(),
                          MakeAddressAccessor(&RomamSink::m_local),
                          MakeAddressChecker())
            .AddAttribute("Protocol",
                          "The type id of the protocol to use for the rx socket.",
                          TypeIdValue(UdpSocketFactory::GetTypeId()),
                          MakeTypeIdAccessor(&RomamSink::m_tid),
                          MakeTypeIdChecker())
            .AddAttribute("EnableSeqTsSizeHeader",
                          "Enable optional header tracing of SeqTsSizeHeader",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RomamSink::m_enableSeqTsSizeHeader),
                          MakeBooleanChecker())
            .AddTraceSource("Rx",
                            "A packet has been received",
                            MakeTraceSourceAccessor(&RomamSink::m_rxTrace),
                            "ns3::Packet::AddressTracedCallback")
            .AddTraceSource("RxWithAddresses",
                            "A packet has been received",
                            MakeTraceSourceAccessor(&RomamSink::m_rxTraceWithAddresses),
...
// void
// RomamSink::MonitorThroughput (uint32_t interval)
// {
...
Ptr<Socket>
RomamSink::GetListeningSocket(void) const
{
    NS_LOG_FUNCTION(this);
    return m_socket;
}
...
std::list<Ptr<Socket>>
RomamSink::GetAcceptedSockets(void) const
{
    NS_LOG_FUNCTION(this);
    return m_socketList;
}
...
Time
RomamSink::GetDelay(const Ptr<Packet>& p) const
{
    NS_LOG_FUNCTION(this);
...
// Application Methods
void
RomamSink::StartApplication() // Called at time specified by Start
{
    NS_LOG_FUNCTION(this);
    ...
    m_socket->SetRecvCallback(MakeCallback(&RomamSink::HandleRead, this));
    m_socket->SetAcceptCallback(MakeNullCallback<bool, Ptr<Socket>, const Address&>(),
                                MakeCallback(&RomamSink::HandleAccept, this));
    m_socket->SetCloseCallbacks(MakeCallback(&RomamSink::HandlePeerClose, this),
                                MakeCallback(&RomamSink::HandlePeerError, this));
}

void
RomamSink::StopApplication() // Called at time specified by Stop
{
    NS_LOG_FUNCTION(this);
...
void
RomamSink::HandleRead(Ptr<Socket> socket)
{
...
void
RomamSink::PacketReceived(const Ptr<Packet>& p, const Address& from, const Address& localAddress)
{
...
void
RomamSink::HandlePeerClose(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
}

void
RomamSink::HandlePeerError(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
}

void
RomamSink::HandleAccept(Ptr<Socket> s, const Address& from)
{
    NS_LOG_FUNCTION(this << s << from);
    s->SetRecvCallback(MakeCallback(&RomamSink::HandleRead, this));
