...
NS_LOG_COMPONENT_DEFINE("TestQueueDisc");

NS_OBJECT_ENSURE_REGISTERED(TestQueueDisc);

TypeId
TestQueueDisc::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::TestQueueDisc")
            .SetParent<QueueDisc>()
...
void
TestQueueDisc::SetBurst(uint32_t burst)
{
...
void
TestQueueDisc::SetMtu(uint32_t mtu)
{
...
void
TestQueueDisc::SetRate(DataRate rate)
{
...
void
TestQueueDisc::SetPeakRate(DataRate peakRate)
{
...
DataRate
TestQueueDisc::GetPeakRate() const
{
    NS_LOG_FUNCTION(this);
    return m_peakRate;
}
...
uint32_t
TestQueueDisc::GetFirstBucketTokens() const
{
    NS_LOG_FUNCTION(this);
    return m_btokens;
}
...
uint32_t
TestQueueDisc::GetSecondBucketTokens() const
{
    NS_LOG_FUNCTION(this);
    return m_ptokens;
}
...
bool
TestQueueDisc::CheckConfig()
{
    NS_LOG_FUNCTION(this);
    if (GetNInternalQueues() > 0)
    {
        NS_LOG_ERROR("TestQueueDisc cannot have internal queues");
        return false;
    }

    if (GetNPacketFilters() > 0)
    {
        NS_LOG_ERROR("TestQueueDisc cannot have packet filters");
        return false;
    }
    ...
    if (GetNQueueDiscClasses() == 0)
    {
        ...
        {
            NS_LOG_ERROR(
                "Cannot set the max size of the child queue disc to that of TestQueueDisc");
            return false;
        }
        qd->Initialize();
    ...
    {
        NS_LOG_ERROR("TestQueueDisc needs 1 child queue disc");
        return false;
    }
...
void
TestQueueDisc::InitializeParams()
{
    NS_LOG_FUNCTION(this);
