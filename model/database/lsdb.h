...
const uint32_t INFINITY_N = 0xffffffff; //!< "infinite" distance between nodes
...
/**
 * @brief Get the Router Link State Advertisement returned by the 
 * Router represented by this Vertex during the route discovery 
 * process.
 *
 * @see Router
 * @see RoutingLSA
...
/**
 * @brief Set the Router Link State Advertisement returned by the 
 * Router represented by this Vertex during the route discovery 
 * process.
 *
 * @see Vertex::GetLSA ()
 ...
 * to begin forwarding packets from the root DGRVertex to "this" Vertex.
 *
 ...
 */
  void SetRootExitDirection (Ipv4Address nextHop, int32_t id = INFINITY_N);
...
/**
 * @brief Get a pointer to the Vector that is the parent of "this" 
 * Vertex.
 *
...
private:
  ...
  typedef std::list<Vertex*> ListOfVertex_t; //!< container of DGRVertexes
