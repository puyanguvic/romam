...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run containerlab link-delay coverage experiment and export probe metrics."
    )
    ...
    parser.add_argument(
        "--run-output-dir",
        default="results/runs/ospf_coverage_containerlab",
...
def run_once(
    ...
    run_id = f"ospf_coverage_containerlab_n{args.n_nodes}_r{run_idx}_{now_tag()}"
    ...
    run_payload = {
        "run_id": run_id,
        "name": f"ospf_coverage_containerlab_n{args.n_nodes}_r{run_idx}",
        "seed": seed,
...
def summarize(metrics_rows: List[Dict[str, Any]], args: argparse.Namespace) -> Dict[str, Any]:
    return {
        "experiment": "ospf_coverage_containerlab_exp",
...
def main() -> None:
    ...
    prefix = args.result_prefix or f"results/tables/ospf_coverage_containerlab_n{args.n_nodes}"
    json_path, csv_path = save_outputs(summary, prefix)

    print("=== OSPF Coverage Containerlab Experiment ===")
